...
import { normalizeRole, ROLES } from '#core/security/roles.js';
...
export async function authMiddleware(req, res, next) {
  try {
    ...
    if (role === ROLES.ASESOR && usuario.asesor?.estado !== 'ACTIVO') {
      return res.status(403).json({ error: 'Asesor inactivo', code: 'ADVISOR_INACTIVE', requestId: req.id });
    }
    const requestPlatform = req.get('x-client-platform') === 'mobile' ? 'mobile' : 'web';
    if (role === ROLES.ASESOR && (decoded.channel !== 'mobile' || requestPlatform !== 'mobile')) {
      return res.status(403).json({ error: 'El acceso del asesor está permitido únicamente desde el aplicativo móvil', code: 'MOBILE_APP_REQUIRED', requestId: req.id });
    }
    req.user = {
      id: usuario.id_usuario, username: usuario.username,
      rol: role, id_asesor: usuario.id_asesor,
    };
    next();
  } catch (error) {
    next(error);
